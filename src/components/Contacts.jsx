...
export default function Contacts() {
  return (
    <section style={{ padding: '0 64px 64px', display: 'flex', flexDirection: 'column', gap: 48 }}>
      <h2 style={{ fontSize: 51, fontWeight: 700, margin: 0 }}>Поддержка</h2>
      <div style={{ display: 'flex', gap: 24 }}>
        {CONTACTS.map((c, i) => (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 24 }}>
            {ICONS[i]}
            <a href={c.href} style={{ fontSize: 28, fontWeight: 700, color: '#000' }}>
              {c.title}
            </a>
            <div style={{ fontSize: 16, fontWeight: 500, color: 'rgba(0,0,0,0.75)' }}>{c.sub}</div>
          </div>
        ))}
      </div>
    </section>
